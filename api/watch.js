...
module.exports = (req, res) => {
    ...
    /* DETECCIÓN AUTOMÁTICA DE LINK TIPO 1 (autenticación por query string)
       En vez de detectar parámetros específicos (que varían por servidor),
       detectamos si el .m3u8 tiene CUALQUIER query string con parámetros.
       Si tiene ?algo=valor → pasa por el proxy.
       Si no tiene query string o solo tiene ? vacío → va directo.
       Esto cubre todas las variaciones posibles sin mantenimiento. */
    const urlObj       = (() => { try { return new URL(urlDecoded); } catch(err) { return null; } })();
    const hasAuthQuery = isM3u8 && urlObj && urlObj.searchParams.toString().length > 0;
    const finalUrl     = hasAuthQuery
        ? `/api/proxy?url=${encodeURIComponent(urlDecoded)}`
        : urlDecoded;
    ...
    const safeFinalUrl = finalUrl.replace(/"/g, '&quot;');
...
<link rel="preload" href="${safeUrl}" as="fetch">
<style>
...
<div class="player-wrap" style="position:relative;z-index:1">
  <video id="v" controls playsinline preload="auto"
         poster="${thumb}"${isM3u8 ? '' : ` src="${safeFinalUrl}"`}>
    ${isM3u8 ? '' : `<source src="${safeFinalUrl}" type="video/mp4">`}
  </video>
</div>
<div id="errBox" style="display:none;background:#1a0000;border:1px solid #ff4d4d;color:#ff4d4d;padding:12px 16px;font-size:0.8rem;font-family:monospace;word-break:break-all;margin:8px;border-radius:6px;"></div>
...
var isM3u8=${isM3u8};
var VU="${safeFinalUrl}";
/* Test rápido del proxy antes de intentar reproducir */
var errBox=document.getElementById('errBox');
if(VU.startsWith('/api/proxy')){
    fetch(VU)
        .then(function(r){
            if(!r.ok) throw new Error('Proxy respondió HTTP '+r.status);
            return r.text();
        })
        .then(function(txt){
            if(!txt.includes('#EXTM3U')){
                if(errBox){ errBox.style.display='block'; errBox.innerText='❌ Proxy: respuesta no es HLS válido. Primeros 200 chars: '+txt.substring(0,200); }
            }
        })
        .catch(function(err){
            if(errBox){ errBox.style.display='block'; errBox.innerText='❌ Proxy falló: '+err.message; }
        });
}
if(isM3u8){
    ...
    s.onload=function(){
        ...
        if(typeof Hls!=='undefined'&&Hls.isSupported()){
            var h=new Hls({xhrSetup:function(xhr){xhr.withCredentials=false;}});
            h.loadSource(VU);
            h.attachMedia(v);
            h.on(Hls.Events.ERROR,function(ev,data){
                if(data.fatal){
                    /* Mostrar error visible en pantalla con detalles */
                    if(errBox){
                        errBox.style.display='block';
                        errBox.innerText='❌ Error HLS: '+data.type+' / '+data.details+' — URL: '+VU.substring(0,80)+'...';
                    }
                }
            });
            ...
            v.src=VU;
            v.onerror=function(){
                if(errBox) errBox.style.display='block', errBox.innerText='❌ Error nativo: '+v.error.message;
            };
        }
    };
    s.onerror=function(){
        var errBox=document.getElementById('errBox');
        if(errBox) errBox.style.display='block', errBox.innerText='❌ Error: no se pudo cargar hls.js';
    };
