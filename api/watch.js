...
var isM3u8=${isM3u8};
var VU="${safeFinalUrl}";
if(isM3u8){var s=document.createElement('script');s.src='https://cdn.jsdelivr.net/npm/hls.js@latest';s.onload=function(){var v=document.getElementById('v');if(typeof Hls!=='undefined'&&Hls.isSupported()){var h=new Hls({xhrSetup:function(xhr){xhr.withCredentials=false;}});h.loadSource(VU);h.attachMedia(v);h.on(Hls.Events.ERROR,function(ev,data){console.warn('HLS error:',data);});}else if(v.canPlayType('application/vnd.apple.mpegurl')){v.src=VU;}};document.head.appendChild(s);}
